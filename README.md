...
배포된 사이트를 핸드폰 브라우저로 연 뒤:
- **아이폰(Safari)**: 공유 버튼 → **홈 화면에 추가**
- **안드로이드(Chrome)**: 메뉴(⋮) → **앱 설치 / 홈 화면에 추가**

홈 화면의 LUX 아이콘으로 열면 주소창 없이 전체화면 앱처럼 실행됩니다.
(설치 조건인 웹 매니페스트·아이콘·서비스워커가 모두 포함되어 있습니다. 단, PWA 설치는 **https 로 배포된 주소**에서만 됩니다 — 로컬 `npm run dev` 는 미리보기용입니다.)
